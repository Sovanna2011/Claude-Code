...
//
// The table name is interpolated, not bound: it never comes from a request, only from the
// repository method that already knows which table it just wrote to.
func explainMiss(ctx context.Context, q database.Querier, table, entity string, id int) error {
	var exists bool
...
func (r *MasterRepository) explainMiss(ctx context.Context, tx database.Querier, table, entity string, id int) error {
	return explainMiss(ctx, r.q(tx), table, entity, id)
}
