...
| Q17 | The `INTEGRATION` machine account holds `plan:read` so a gate ticket can be resolved to its season and version. `plan:read` is also the permission that gates stock, downtime, quality samples and production orders, so a weighbridge terminal can read those screens. Should interfaces have a narrower read permission of their own? | Open. Nothing is writable through it and no fix was rushed in: splitting a read permission touches every read handler, and the answer depends on whether the mill wants terminals to see stock at all. Recorded rather than quietly widened or quietly narrowed. |
